...
# 
# Example usage:
#  samtools view sample.bam chr4:55589744-55589911 | ./indelseek_ram.pl
# 
...
LINE:while (<>) {
	...
	if ($line_counter % 100000 == 0) { # print number of processed lines every 100000 lines
        print "Processed $line_counter lines\n";
    }

    if (has_one_minute_passed($ram_time)) {
        log_ram_usage();

        # Reset the start time for the next minute
        $ram_time = gettimeofday();
    }
...
sub log_ram_usage {
    ...
    # Read the process status from the /proc file system
    my $status = read_file("/proc/$pid/status");

    # Extract the RSS and VSZ memory usage from the status
    my ($rss_memory) = $status =~ /VmRSS:\s+(\d+)/;
    my ($vsz_memory) = $status =~ /VmSize:\s+(\d+)/;

    # Get the current timestamp
    my $timestamp = localtime;

    # Write the memory usage and timestamp to the log file
    print $log_fh "Timestamp: $timestamp\n";
    print $log_fh "RSS Memory Usage: $rss_memory kB\n";
    print $log_fh "VSZ Memory Usage: $vsz_memory kB\n";
    print $log_fh "-----------------------\n";

    # Flush the buffer to ensure immediate writing to the file
    $log_fh->flush();
}

sub has_one_minute_passed {
    my ($input_time) = @_;
    ...
    my $elapsed_time = $current_time - $input_time;
...
sub faidx {
    ...
    my $newRegion = "chr$region";
    my $output = `$samtools faidx $refseq $newRegion 2>&1`;
    my @outputlines = split("\n",$output);
    die "Error: unexpected output from samtools faidx" unless $outputlines[0] eq ">$newRegion";
...
sub depth {
    ...
    my $newRegion = "chr$region";
    my $output = `$samtools depth -d $max_samtools_depth -r $newRegion -q $quality_threshold $depth_bam`;
    my @outputlines = split("\n",$output);
    my @depth;
...
# Print the runtime of the script in hours
my $elapsed_time = ($end_time - $start_time) / 3600;
print "\n\nThe script ran in: " . sprintf("%.2f", $elapsed_time) . " hours\n";
